...
try:
    ...
    logging.warning(
        "HEIF support not available - install pillow-heif for iPhone image support"
    )
...
DEFAULT_MODEL_STR = "claude-sonnet-4-6"
# </important_do_not_delete>
    ...
    try:
        ...
        try:
            ...
            if magic_bytes.startswith(b"\xff\xd8\xff"):  # JPEG
                ...
                magic_bytes.startswith(b"\x00\x00\x00") and b"ftyp" in magic_bytes
            ):  # HEIC/HEIF
                logging.debug("Detected HEIC/HEIF format")
            else:
                logging.error(f"Unknown image format. Magic bytes: {magic_bytes.hex()}")
                raise ValueError(
                    "Unsupported image format. Please use JPEG, PNG, GIF, WebP, or HEIC images."
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Could not process image file. Error: {str(img_error)}"
                )
        ...
        logging.debug(
            f"Image processed successfully. Original size: {img.size}, Format: {img.format}"
        )
        ...
        logging.error(
            f"Image data type: {type(image_data)}, size: {len(image_data) if hasattr(image_data, '__len__') else 'unknown'}"
        )
        raise ValueError(
            "Unable to process the uploaded image. Please try a different image file."
        )
    ...
    try:
        ...
        for person in people:
            ...
            if person_totals[person] > 0 and total_assigned > 0:
                proportion = person_totals[person] / (
                    total_assigned
                    + (tax * total_assigned / subtotal if subtotal > 0 else 0)
                    + (tip * total_assigned / subtotal if subtotal > 0 else 0)
                )
                base_amount = (
                    person_totals[person]
                    - (
                        tax * person_totals[person] / total_assigned
                        if total_assigned > 0
                        else 0
                    )
                    - (
                        tip * person_totals[person] / total_assigned
                        if total_assigned > 0
                        else 0
                    )
                )
        ...
        return jsonify(
            {"splits": results, "total_split": round(sum(person_totals.values()), 2)}
        )
